...
class Validator {
     ...
     * @returns {Validator}
     */
    #createChildValidator(currentValidatorContext, contextValue, contextValuePath, contextValueCurrentPath, errorContextPath) {
        let validatorState = this.#validatorState.cloneWith(Validator.#validatorStatePool.get(), contextValue, contextValuePath, contextValueCurrentPath, errorContextPath);
     ...
     * the scope of the "correct" property path. Using this method the error context path
     * used for mapping paths to errors can be modified for current validator and child-validators
     * by adding a custom path which will be added to the error prefix path (if set).
     * The path can be whatever makes sense.
     *
     * @example
     * let test = Validator.create();
     * let person = { email: '', username: 'john' };
     * test(person).errorContext('emailAndUsername').fulfillOneOf((name) => [ // make a new error context called emailAndUsername
     *      name.isNot.empty(),
     *      email.isNot.empty()
     * ], '"email" or "username" must be filled out');
     *
     * // result will map errors to ['emailAndUsername' => '"email" or "username" must be filled out']
     * instead of ['' => '"email" or "username" must be filled out']
     *
     * @param {string} errorPrefixPath the path to prefix the errors from the validation branch with
     * @returns {Validator}
     */
    errorContext(errorPrefixPath) {
        if (!_.isString(errorPrefixPath)) {
            Validator.#throwArgumentError('errorPrefixPath must be string');
        }
        let validatorContext = this.#getValidatorContext(false);

        let validator = this.#createChildValidator(validatorContext, this.#contextValue, this.#contextValuePath, this.#contextValueCurrentPath, errorPrefixPath);
        // important to call this to make sure reset() is called and the context is returned to the contextPool, because we are leaving this context and enter a child validator
        // IMPORTANT that we pass undefined as success so we don't modify short circuit state etc. as getting a prop is NOT a predicate
        this.#validatorContextDone(validatorContext, undefined);
        return validator;
