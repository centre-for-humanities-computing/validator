...
class Validator {
    ...
    /**
     * @param {boolean} notContext set to `true` if this is a notContext otherwise `false`
     ...
     * @param {string} [contextValuePath] the contextValuePath for this validator if `undefined` the value from the parent will be used
     * @param {string} [contextValueCurrentPath] the contextValueCurrentPath for this validator if `undefined` the value from the parent will be used
    ...
    /**
     * Tests the following predicate *only* if this value is defined. (not `null` and not `undefined`).
     *
    ...
    /**
     * Tests the following predicate *only* if this value is not `null`.
     *
     * Descendant predicates added with {@link #each}, {@link ValidatorContext#fulfill}, {@link ValidatorContext#fulfillAllOf},
     * {@link ValidatorContext#fulfillOneOf} will as well only be tested if this value is not `null`.
     *
     * @example
     * let test = Validator.create();
     * let person = null;
     * // the below tests will only be performed if person is not `null`
     * test(person).optionalNull.fulfillAllOf(person => [
     *      person.is.anObject('person must be an object'),
     *      person.prop("name").fulfillAllOf(name => [
     *          name.is.aString('"${PATH}" must be a string'),
     *          name.does.match(/\w+/, '"${PATH}" must only contain [a-Z_0-9]')
     *      ]),
     *      // will only be tested if person is not `null` and `person.age` is not `null`
     *      test(person).optionalNull.prop("age").optionalNull.is.aNumber('"${PATH}" must be a number')
     * ]);
     *
     * @returns {Validator} This instance set to optional mode.
     */
    get optionalNull() {
        if (this.#contextValue === null) {
            this.#contextShortCircuit.fulfilled = true;
            if (Debug.enabled) {
                this.#printDebug('{?}', 'optionalNull', 'bypassed branch because of null value');
            }
        }
        return this;
    }
    ...
    /**
     * Tests the following predicate *only* if this value is not `undefined`.
     *
     * Descendant predicates added with {@link #each}, {@link ValidatorContext#fulfill}, {@link ValidatorContext#fulfillAllOf},
     * {@link ValidatorContext#fulfillOneOf} will as well only be tested if this value is not `undefined`.
     *
     * @example
     * let test = Validator.create();
     * let person = undefined;
     * // the below tests will only be performed if person is not `undefined`
     * test(person).optionalUndefined.fulfillAllOf(person => [
     *      person.is.anObject('person must be an object'),
     *      person.prop("name").fulfillAllOf(name => [
     *          name.is.aString('"${PATH}" must be a string'),
     *          name.does.match(/\w+/, '"${PATH}" must only contain [a-Z_0-9]')
     *      ]),
     *      // will only be tested if person is not `undefined` and `person.age` is not `undefined`
     *      test(person).optionalUndefined.prop("age").optionalUndefined.is.aNumber('"${PATH}" must be a number')
     * ]);
     *
     * @returns {Validator} This instance set to optional mode.
     */
    get optionalUndefined() {
        if (this.#contextValue === undefined) {
            this.#contextShortCircuit.fulfilled = true;
            if (Debug.enabled) {
                this.#printDebug('{?}', 'optionalUndefined', 'bypassed branch because of undefined value');
            }
        }
        return this;
    }
     ...
     * @returns {boolean} `true` if all elements passed the predicate test otherwise `false`.
     */
